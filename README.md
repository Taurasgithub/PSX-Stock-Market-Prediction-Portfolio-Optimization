...
Daily OHLCV (Open, High, Low, Close, Volume) history for the selected KSE-100 companies, spanning July 2016 - July 2026, is fetched live from PSX via the [`psxdata`](https://psxdata.mintlify.app/) library rather than from local CSV files. `src/data_loader.py`:

- `fetch_stock(ticker, start, end)` wraps `psxdata.stocks(...)`, normalizing its output to a `Date`-indexed `Open/High/Low/Close/Volume` dataframe.
- `load_portfolio_data(tickers, start, end)` fetches multiple tickers and combines their closing prices into one wide dataframe.
