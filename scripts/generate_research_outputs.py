...
- PORTFOLIO TIER (`run_portfolio_tier`): only needs numpy / pandas /
  scipy / matplotlib / scikit-learn plus live data fetched via `psxdata`.
...
# The 10 selected KSE-100 tickers and the 10-year data timeframe, fetched
# live from PSX via the `psxdata` library (see src/data_loader.py).
TICKERS: List[str] = ["ENGRO", "SYS", "LUCK", "HUBC", "TRG"]
# Add the remaining companies' tickers here.
...
def load_real_returns(
    tickers: List[str] = TICKERS, start: str = START_DATE, end: str = END_DATE
) -> pd.DataFrame:
    """Fetch the project's real multi-stock closing-price history from
    PSX (via `src.data_loader.load_portfolio_data`, which wraps
    `psxdata.stocks`) and convert it to daily simple returns."""
    prices = load_portfolio_data(tickers, start=start, end=end)
...
def run_portfolio_tier(
    ...
    predicted_returns : pd.DataFrame, optional
        ...
        accordingly.
    tickers, start, end, rebalance_frequency, cov_lookback, trading_days :
    ...
    realized_returns = load_real_returns(tickers, start=start, end=end)
...
def run_modeling_tier(
    representative_ticker: str = "LUCK",
    start: str = START_DATE,
    end: str = END_DATE,
    window: int = 20,
    epochs: int = 100,
    ...
    ohlcv = fetch_stock(representative_ticker, start=start, end=end)
